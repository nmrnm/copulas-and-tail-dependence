...
for(j in 1:numSimulations){
  eps_res <- res[sample(nrow(res),size=timeSteps,replace=T),] 
  eps_com <- com_mat[sample(nrow(res),size=timeSteps,replace=T),] 
  eps_new_res <- newMultiNormMarg[sample(nrow(newMultiNormMarg),size=timeSteps,replace=T),] 

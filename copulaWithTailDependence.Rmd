...
```{r}
eps_com <- rep(scale_eps*sample(com/dim)/range(com/dim),times=dim)
eps_res <- scale_eps*sample(res[,1])/range(res[,1])
eps_new_res <- scale_eps*sample(newMultiNormMarg[,1])/range(newMultiNormMarg[,1])

for(i in 2:dim){
  eps_res <- scale_eps*sample(res[,i])/range(res[,i])
  eps_new_res <- scale_eps*sample(newMultiNormMarg[,1])/range(newMultiNormMarg[,1])
}
```

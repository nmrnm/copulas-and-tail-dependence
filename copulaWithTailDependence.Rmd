...
```{r}
eps <- res[sample(nrow(res),size=1000,replace=TRUE),]
popSize <- matrix(data=rep(initPopSize,times=dim),nrow=1,byrow = T)

for(i in 1:1000){
  popSize<- rbind(popSize,ricker(popSize[i,],r,carryingCapacity,eps[i,]))
}
popSize <- rowSums(popSize)
plot.ts(popSize)
hist(popSize,50)
```

```{r}
corMatPopCom <- matrix(data=rep(0,times=dim*dim),nrow=dim)
corMatPopRes <- matrix(data=rep(0,times=dim*dim),nrow=dim)
corMatPopNewRes <- matrix(data=rep(0,times=dim*dim),nrow=dim)
for(i in 1:dim){
  for(j in 1:dim){
    corMatPopCom[i,j] <- cor(popSizeCom[,i],popSizeCom[,j])
    corMatPopRes[i,j] <- cor(popSizeRes[,i],popSizeRes[,j])
    corMatPopNewRes[i,j] <- cor(popSizeNewRes[,i],popSizeNewRes[,j])
  }
}
# print(corMatPopCom)
# print(corMatPopRes)
# print(corMatPopNewRes)
```
...
```{r}
#will add back later
# popSizeCom[i,] <- replace(popSizeCom[i,], popSizeCom[i,] < tLow | popSizeCom[i,] > tHigh, 0)
      # popSizeRes[i,] <- replace(popSizeRes[i,], popSizeRes[i,] < tLow | popSizeRes[i,] > tHigh, 0)
      # popSizeNewRes[i,] <- replace(popSizeNewRes[i,], popSizeNewRes[i,] < tLow | popSizeNewRes[i,] > tHigh, 0)
```

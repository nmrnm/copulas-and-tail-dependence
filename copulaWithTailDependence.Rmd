...
newCopulaWithTailDependece<-function(res,n,dim,rho,thresh_lo,thresh_hi)
{
  U<-runif(n)
...
upper<-newCopulaWithTailDependece(res,params$n,params$dim,params$rho,0,1-params$thresh)
lower<-newCopulaWithTailDependece(res,params$n,params$dim,params$rho,params$thresh,1)
both<-newCopulaWithTailDependece(res,params$n,params$dim,params$rho,params$thresh/2,1-(params$thresh/2))
```
...
```{r}
#arbitrarily chose upper, it makes no difference
varSumRetain <- sum(cov(upper))
...
* <b>comonotonic</b> in the both tails: `r sum(cov(both))` `r var(sum_both)`
...
* <b>comonotonic</b> in the both tails: `r var(both[,1])`
